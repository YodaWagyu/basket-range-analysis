...
def generate_ai_insight(df, category, brand):
    ...
    prompt = f"""
    คุณคือ Business Data Analyst ที่แม่นยำเรื่อง Logic และใช้ภาษาเข้าใจง่าย
    ...
    1.  **Hero Range:** คือช่วงราคาที่มี **% Share Sales สูงสุด** (ตัวทำเงินหลัก)
    2.  **Traffic Generator:** คือช่วงราคาที่ต่ำกว่า Hero Range แต่มี **% Share Bills สูง** (ตัวดึงคนเข้าร้านด้วยของถูก)
    3.  **The Goal:** เป้าหมายคือการดันลูกค้าจาก Traffic -> Hero และจาก Hero -> Next Step (ราคาที่สูงขึ้นถัดไป) **ห้ามแนะนำให้ดันลูกค้าลงไปซื้อของถูกกว่าเดิม**

    ### ⚠️ ข้อควรระวัง (Constraints):
    1.  **Data Accuracy:** แยกแยะ SPLY (ปีก่อน) และ CURRENT (ปีนี้) ให้แม่นยำ ตรวจสอบว่าตัวเลขเพิ่มขึ้น (+) หรือลดลง (-) กันแน่
    2.  **Context:** ห้ามเอาเป้ายอดขายรวม (เช่น 50M) มาพูดเป็น Action ของลูกค้าหนึ่งคน
    3.  **Human-Readable Numbers (English Units):**
        - ยอดขายใช้หน่วย **M THB** (เช่น 396M THB)
        - บิลใช้หน่วย **M Bills** หรือ **K Bills** (เช่น 1.7M Bills, 960K Bills)

    ---
    ### รูปแบบการตอบ (Output Format):
    ตอบเป็นภาษาไทย แบ่งเป็น 3 หัวข้อ ใช้ Markdown Headings (####) และ Bullet points ห้ามใช้ตัวหนา

    #### Insight
    - ระบุชัดเจนว่า **Hero Range** คือช่วงไหน (Sales เท่าไหร่) และ **Traffic Generator** คือช่วงไหน (Bills เท่าไหร่)
    - เปรียบเทียบ Hero Range ปีนี้ (Current) vs ปีก่อน (SPLY) ว่าเติบโตหรือหดตัวกี่ M THB
    - วิเคราะห์พฤติกรรม: ลูกค้าส่วนใหญ่ยังกองอยู่ที่ Hero หรือเริ่มขยับไป Range ถัดไปแล้ว?

    #### Action
    - **Action 1 (Fix Traffic):** เสนอวิธีเปลี่ยนลูกค้ากลุ่ม Traffic Generator (ของถูก) ให้ขยับยอดซื้อขึ้นมาที่ Hero Range
    - **Action 2 (Stretch Hero):** เสนอวิธีดันลูกค้ากลุ่ม Hero Range ให้ซื้อเพิ่มอีกนิดเพื่อไปแตะ **Range ถัดไปที่สูงขึ้น**
    - **Note:** เน้นกลยุทธ์จัดเซตหรือเพิ่มชิ้น (Basket size mechanics)

    #### Suggest Basket Size
    - แนะนำ **ตัวเลขเดียว** สำหรับทำ GWP (Gift With Purchase)
    - **Logic:** ให้ใช้ **"ราคาเริ่มต้น (Lower Bound) ของ Range ถัดไป"** ที่อยู่ติดกับ Hero Range เท่านั้น
