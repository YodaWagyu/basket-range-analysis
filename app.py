...
def generate_ai_insight(df, category, brand):
    ...
    prompt = f"""
    คุณคือผู้เชี่ยวชาญด้านการค้าปลีกที่เก่งอธิบายข้อมูลให้คนทั่วไปเข้าใจง่ายๆ แบบเพื่อนคุย
    
    📦 สินค้าที่กำลังวิเคราะห์: {category} - {brand}
    
    📊 ข้อมูลที่ได้จากระบบ (ห้ามแต่งเลขเอง ใช้เฉพาะตัวเลขจากตารางนี้):
    ...
    1. หาช่วงราคาที่ทำยอดขายสูงสุด (Hero Range):
       - ดูคอลัมน์ "% Share Sales" ในข้อมูล CURRENT (Year = ปีล่าสุด)
       - ช่วงไหนมี % สูงสุด = นั่นคือ Hero Range
       - เอายอดขาย (TotalSales) จาก CURRENT มาใส่ แล้วแปลงเป็น M THB พร้อมคอมม่า
    
    2. หาช่วงราคาที่มีคนซื้อเยอะที่สุด (Traffic Generator):
       - ดูคอลัมน์ "% Share Bills" ในข้อมูล CURRENT
       - ช่วงไหนมี % สูงสุด = นั่นคือ Traffic Generator
       - เอาจำนวนบิล (TotalBills) จาก CURRENT มาใส่ แล้วแปลงเป็น K Bills พร้อมคอมม่า
    
    3. คำนวณว่า Hero Range โตหรือลดเท่าไหร่:
       - เอา TotalSales ของ Hero Range จาก CURRENT ลบด้วย SPLY
       - ระวัง: ต้องเป็นช่วง (Basket_Range) เดียวกัน
       - แสดงผลเป็น M THB พร้อมคอมม่า
    
    4. แนะนำราคาเป้าหมายสำหรับทำโปรโมชั่น:
       - หาว่า Hero Range คือช่วงไหน (เช่น 99-148)
       - หาช่วงถัดไปที่สูงกว่า (เช่น 149-198)
       - แนะนำตัวเลขเริ่มต้นของช่วงนั้น (เช่น 149)
    
    ---
    
    ✍️ รูปแบบการเขียน (สำคัญมาก):
    
    - ห้ามใช้ตัวหนา (**), ตัวเอียง (*), หรือ underscore (_) เด็ดขาด
    - เขียนแบบเล่าเรื่อง เหมือนคุยกับเพื่อน
    - ใช้ตัวเลขที่มีคอมม่า เช่น "43.1M THB", "117.9K Bills"
    - อธิบายให้เข้าใจง่าย ไม่ต้องใช้คำเทคนิค
    ...
    [เขียนเป็นย่อหน้าเล่าเรื่อง 3-4 ประโยค อธิบายว่า:]
    - ลูกค้าส่วนใหญ่ชอบซื้อช่วงราคาไหน (Hero Range) ยอดขายเท่าไหร่
    - ช่วงราคาไหนมีคนซื้อบ่อยที่สุด (Traffic Generator) กี่บิล
    - เมื่อเทียบกับปีก่อน ช่วงขายดีโตหรือลด กี่บาท
    - ลูกค้ามีพฤติกรรมอย่างไร (ซื้อแพงขึ้นหรือถูกลง)
    
    [ตัวอย่าง: "จากข้อมูลพบว่า ลูกค้าส่วนใหญ่ชอบซื้อในช่วงราคา 99-148 บาท ซึ่งทำยอดขายสูงถึง 43.1M THB แต่ช่วงที่มีคนซื้อบ่อยที่สุดคือช่วง 49-98 บาท มีถึง 117.9K บิล เมื่อเทียบกับปีก่อน ช่วง 99-148 บาทโตขึ้นมาก 19.2M THB แสดงว่าลูกค้าเริ่มซื้อสินค้าที่แพงขึ้น"]
    
    #### Action
    
    [เขียนเป็นย่อหน้า 2 ประโยค แนะนำกลยุทธ์:]
    
    Action 1: [อธิบายง่ายๆ ว่าจะทำยังไงให้คนที่ซื้อช่วงถูก ซื้อเพิ่มมาที่ช่วงขายดี]
    
    Action 2: [อธิบายง่ายๆ ว่าจะทำยังไงให้คนที่ซื้อช่วงขายดีอยู่แล้ว ซื้อเพิ่มให้ถึงช่วงที่แพงกว่า]
    
    [ตัวอย่าง: "ลองทำโปรโมชั่น ซื้อ 2 แถม 1 ให้คนที่ซื้อแค่ 1 ขวด (ช่วง 49-98) ได้ลองซื้อเพิ่มเป็น 2 ขวด เพื่อให้ถึงช่วง 99-148 บาท" และ "สำหรับคนที่ซื้ออยู่ที่ 99-148 บาทแล้ว ให้ของแถมสุดคุ้มเมื่อซื้อครบ 149 บาท เพื่อกระตุ้นให้ซื้อเพิ่มอีกนิด"]
    
    #### Suggest Basket Size
    
    [เขียนเป็นย่อหน้า 2-3 ประโยค:]
    - บรรทัดแรก: แนะนำราคาเป้าหมาย [X] บาท สำหรับทำโปรโมชั่น "ซื้อครบ X บาทรับของแถม"
    - บรรทัดสอง: อธิบายว่าทำไมเลือกราคานี้ (เพราะลูกค้าส่วนใหญ่ซื้ออยู่ที่เท่าไหร่ ต้องซื้อเพิ่มแค่กี่บาท)
    - บรรทัดสาม: เตือนว่าของแถมต้องคุ้มค่า ควรลงทุนประมาณ 15-25% ของจำนวนที่ต้องซื้อเพิ่ม
    
    [ตัวอย่าง: "แนะนำให้ตั้งราคาโปรโมชั่นที่ 149 บาท เพื่อใช้เป็นเงื่อนไข ซื้อครบ 149 บาทรับของแถม เพราะลูกค้าส่วนใหญ่ซื้ออยู่ที่ประมาณ 107 บาท ถ้าซื้อเพิ่มแค่ 42 บาทก็ได้ของแถม ไม่ยากเกินไป สำหรับของแถม ควรลงทุนประมาณ 6-10 บาท (15-25% ของ 42 บาท) และเลือกของที่ลูกค้ารู้สึกคุ้มค่า"]
    
    ---
    
    ⚠️ เช็กก่อนส่งคำตอบ:
    - ทุกตัวเลขมาจากตารางข้างต้นจริงหรือไม่?
    - ใส่คอมม่าในตัวเลขทุกตัวหรือยัง?
    - การคำนวณผลต่าง CURRENT - SPLY ถูกต้องหรือไม่?
    - Suggest Basket Size เป็นช่วงถัดไปจาก Hero Range หรือเปล่า?
    - เขียนแบบเล่าเรื่อง ไม่มี ** หรือ * หรือไม่?
    """
    ...
    try:
        res = client.chat.completions.create(
            model="gpt-4o-mini",
            messages=[
                {"role": "system", "content": "คุณคือที่ปรึกษาด้านการค้าปลีกที่อธิบายข้อมูลได้ง่ายๆ เหมือนเพื่อนคุย ไม่ใช้คำเทคนิค"},
