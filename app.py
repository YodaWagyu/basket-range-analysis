...
def generate_ai_insight(df, category, brand):
    ...
    📊 ข้อมูล (ห้ามคำนวณมั่ว ใช้เลขจากตารางนี้เท่านั้น):
    {lean_df.to_string(index=False)}
    
    ---
    
    🧠 วิธีคิด (Logic):
    1. **Hero Range (ขายดีสุด):** ช่วงราคาที่มี % Share Sales สูงสุดในฝั่ง CURRENT
    2. **Traffic Generator (คนซื้อบ่อยสุด):** ช่วงราคาที่มี % Share Bills สูงสุดในฝั่ง CURRENT (และราคาต่ำกว่า Hero)
    3. **การเติบโต:** เอา Sales ของ Hero Range (CURRENT) ลบด้วย (SPLY)
    4. **เป้าหมายโปรโมชั่น:** ราคาเริ่มต้นของ "Range ถัดไป" ที่อยู่ติดกับ Hero Range
    ...
    4. **ตัวเลข:** - ยอดขายใช้หน่วย M THB (เช่น 43.1 M THB)
       - บิลใช้หน่วย K Bills (เช่น 117.9 K Bills)
    ...
    - 📈 เทรนด์ปีนี้: ช่วง Hero Range ยอดขาย [โตขึ้น/ลดลง] [ตัวเลข] M THB เมื่อเทียบกับปีก่อน
    ...
    - 🗣️ เหตุผล: [อธิบายว่าลูกค้า Hero ต้องซื้อเพิ่มอีกกี่บาทถึงจะได้ของแถม ซึ่งเป็นยอดที่เอื้อมถึง]
    - 🎁 คำแนะนำของแถม: [แนะนำมูลค่าของแถมที่เหมาะสม (ประมาณ 15-20% ของส่วนต่างที่ต้องจ่ายเพิ่ม)]
    """
