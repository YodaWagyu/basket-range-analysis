...
def generate_ai_insight(df, category, brand):
    ...
    Data (ใช้ข้อมูลจากตารางนี้เท่านั้น ห้ามคำนวณเอง):
    {lean_df.to_string(index=False)}

    ---
    ### 🛑 กฎเหล็กเรื่องข้อมูลและการแสดงผล (Strict Constraints):
    1.  **ห้ามใช้ตัวหนา (No Bold):** ห้ามใส่เครื่องหมาย ** หรือ __ ในคำตอบเด็ดขาด
    2.  **หน่วยตัวเลข (Units):**
        - ยอดขาย: ปัดเศษและใช้หน่วย **M THB** (เช่น 43.1M THB)
        - จำนวนบิล: ปัดเศษและใช้หน่วย **K Bills** หรือ **M Bills** (เช่น 117K Bills)
        - *ห้ามพิมพ์เลขดิบยาวๆ*
    3.  **เลือกข้อมูลให้ถูกปี:**
        - **Insight ทั้งหมดต้องมาจากฝั่ง CURRENT (ปีล่าสุด) เท่านั้น**
        - ใช้ฝั่ง SPLY (ปีก่อน) เฉพาะตอนเปรียบเทียบว่าเพิ่ม/ลด เท่านั้น ห้ามเอาเลขปีก่อนมาตอบเป็นสถานะปัจจุบัน
    ...
    #### Insight
    - ระบุ "Hero Range" (ช่วงที่ % Share Sales สูงสุดในฝั่ง Current) ว่าคือช่วงไหน ยอดขายเท่าไหร่
    - ระบุ "Traffic Generator" (ช่วงที่ % Share Bills สูงสุดในฝั่ง Current) ว่าคือช่วงไหน จำนวนบิลเท่าไหร่
    - เปรียบเทียบ Hero Range ปีนี้ vs ปีก่อน ว่ายอดขายเติบโตหรือลดลง (ระบุเป็น + หรือ - กี่ M THB)
    - วิเคราะห์พฤติกรรมลูกค้ากลุ่ม Mass สั้นๆ ว่ามีการขยับช่วงราคา (Trade up/down) อย่างไร

    #### Action
    - Action 1: เสนอวิธีดึงลูกค้าจากกลุ่ม Traffic Generator (Range ต่ำกว่า) ให้ขึ้นมาซื้อที่ Hero Range
    - Action 2: เสนอวิธีดึงลูกค้าจากกลุ่ม Hero Range ให้ซื้อเพิ่มเพื่อแตะ **Range ถัดไปที่สูงขึ้น** (Next Step)
    - *ห้ามแนะนำกว้างๆ ให้เน้นกลยุทธ์เพิ่มยอดต่อบิล*

    #### Suggest Basket Size
    - แนะนำตัวเลข **"ราคาเริ่มต้น (Lower Bound) ของ Range ถัดไป"** ที่อยู่ติดกับ Hero Range
