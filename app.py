...
def generate_ai_insight(df, category, brand):
    ...
    📊 DATA TABLE (อ่านอย่างละเอียด - ห้ามคำนวณเอง):
    {lean_df.to_string(index=False)}

    ---
    ### 🛑 กฎเหล็กในการวิเคราะห์ (CRITICAL RULES):
    
    1. ข้อมูลและการคำนวณ:
       - ใช้เฉพาะตัวเลขจากตาราง DATA TABLE ข้างต้นเท่านั้น
       - ห้ามคำนวณ หรือประมาณการ หรือสร้างตัวเลขเองเด็ดขาด
       - ถ้าไม่มีข้อมูลในตาราง ห้ามตอบ
       - ตรวจสอบความถูกต้องของตัวเลขทุกครั้งก่อนตอบ
       - เมื่อคำนวณผลต่าง: นำตัวเลข CURRENT ลบด้วย SPLY (ห้ามคำนวณผิด)
    
    2. การแสดงผลตัวเลข (NUMBER FORMATTING):
       - ยอดขาย (TotalSales): ต้องมีเครื่องหมายคอมม่าคั่นหลักพัน เช่น "43,109,451" แล้วแปลงเป็น "43.1M THB"
       - จำนวนบิล (TotalBills): ต้องมีเครื่องหมายคอมม่าคั่นหลักพัน เช่น "404,428" แล้วแปลงเป็น "404K Bills"
       - ห้ามแสดงเลขดิบยาวๆ แบบไม่มีคอมม่า
       - ตัวอย่างที่ถูก: "43.1M THB", "404K Bills", "117.9K Bills"
       - ตัวอย่างที่ผิด: "43109451", "23926M THB" (ต้องเป็น "23.9M THB")
    
    3. การเลือกข้อมูล:
       - Hero Range = Basket_Range ที่มี % Share Sales สูงสุดใน CURRENT YEAR
       - Traffic Generator = Basket_Range ที่มี % Share Bills สูงสุดใน CURRENT YEAR
       - เปรียบเทียบ CURRENT vs SPLY: 
         * หา Hero Range ใน CURRENT
         * หา TotalSales ของ Hero Range นั้นใน CURRENT
         * หา TotalSales ของ Hero Range เดียวกันใน SPLY
         * คำนวณผลต่าง = CURRENT - SPLY (ต้องคำนวณให้ถูกต้อง)
       - Suggest Basket Size:
         * หา Hero Range ก่อน (เช่น 99-148)
         * หา Range ถัดไปที่สูงกว่า Hero Range (เช่น ถ้า Hero Range คือ 99-148 ก็ต้องเป็น 149-198)
         * แนะนำตัวเลขเริ่มต้นของ Range นั้น (เช่น 149)
    
    4. การจัดรูปแบบข้อความ:
       - ห้ามใช้ตัวหนา (No ** หรือ __)
       - ห้ามใช้ตัวเอียง (No * หรือ _)
       - ใช้เฉพาะ plain text และ markdown headings (####)

    ---
    ### 📝 รูปแบบการตอบ (OUTPUT FORMAT):
    ตอบเป็นภาษาไทย แบ่งเป็น 3 ส่วน

    #### Insight
    - Hero Range คือ ช่วง [ระบุช่วง] ยอดขาย [ระบุจำนวนพร้อมคอมม่า]M THB (ข้อมูล CURRENT)
    - Traffic Generator คือ ช่วง [ระบุช่วง] จำนวนบิล [ระบุจำนวนพร้อมคอมม่า]K Bills (ข้อมูล CURRENT)
    - เปรียบเทียบ Hero Range ปีนี้ vs ปีก่อน: [CURRENT TotalSales]M THB - [SPLY TotalSales]M THB = เพิ่มขึ้น/ลดลง [คำนวณผลต่างให้ถูกต้อง]M THB
    - วิเคราะห์พฤติกรรม: [สังเกตการเปลี่ยนแปลง % Share Bills ระหว่างช่วงราคา]

    #### Action
    - Action 1: [กลยุทธ์เฉพาะเจาะจงเพื่อดึงลูกค้าจาก Traffic Generator มาซื้อที่ Hero Range]
    - Action 2: [กลยุทธ์เฉพาะเจาะจงเพื่อดึงลูกค้าจาก Hero Range ให้ซื้อเพิ่มไปยังช่วงที่สูงกว่า Hero Range ขึ้นไป 1 ระดับ]

    #### Suggest Basket Size
    - แนะนำราคาเป้าหมาย [ระบุตัวเลขราคาเริ่มต้นของช่วงที่สูงกว่า Hero Range ขึ้นไป 1 ระดับ] THB สำหรับใช้เป็น threshold ของโปรโมชั่น GWP (Gift with Purchase)
    - เหตุผล: [อธิบายว่าทำไมควรกระตุ้นให้ลูกค้าจาก Hero Range ซื้อเพิ่มไปยังช่วงถัดไปที่สูงขึ้น เพื่อเพิ่ม Basket Size และยอดขายรวม โดยอิงจาก ABR และพฤติกรรมการซื้อ]
    - หมายเหตุ: ความสำเร็จของกลยุทธ์นี้ขึ้นอยู่กับมูลค่าและความน่าสนใจของของแถม (GWP) ที่นำมาใช้ ควรเลือกของแถมที่มี perceived value สูงและเหมาะสมกับกลุ่มเป้าหมาย
    
    ---
    ⚠️ สำคัญที่สุด: 
    1. ตรวจสอบทุกตัวเลขกับตารางก่อนตอบ และใส่คอมม่าคั่นหลักพันทุกครั้ง
    2. เมื่อคำนวณผลต่างยอดขาย: ต้องเอา TotalSales CURRENT ลบ TotalSales SPLY ของ Hero Range เดียวกัน แล้วคำนวณให้ถูกต้อง
    3. Suggest Basket Size: ต้องเป็นช่วงที่สูงกว่า Hero Range ขึ้นไป 1 ระดับ ไม่ใช่ Hero Range เอง
       ตัวอย่าง: ถ้า Hero Range = 99-148 ดังนั้น Suggest = 149 (จากช่วง 149-198)
                ถ้า Hero Range = 49-98 ดังนั้น Suggest = 99 (จากช่วง 99-148)
    """
