...
def generate_ai_insight(df, category, brand):
    ...
    1. **ห้ามมโนตัวเลข (No Hallucination):** ตัวเลขเงินบาทที่แนะนำใน Action หรือ GWP ต้องอ้างอิงจาก **ขอบเขตของ 'Basket Range' ที่ปรากฏในตารางเท่านั้น** 2. **ห้ามกระโดดข้ามขั้น (No Quantum Leaps):** หาก Hero Range คือ 149-198 บาท เป้าหมายถัดไปต้องคือ Range ถัดไป (เช่น 199-248) **ห้าม** แนะนำยอด 1,500 หรือ 2,000 บาทเด็ดขาด หากไม่มี Range นั้นที่มี Volume หนาแน่นรองรับ
    3. **Focus on Mass:** สนใจเฉพาะ Basket Range ที่มี **% Share Sales สะสมรวมกันสูงสุด 80% แรก** (ซึ่งมักจะเป็นช่วงราคาไม่สูงมาก) ตัด Range ที่มี % Bills ต่ำกว่า 5% ทิ้งไปจากสมการความคิดทันที

    ---
    ### รูปแบบการตอบ (Output Format):
    ตอบเป็นภาษาไทย แบ่งเป็น 2 หัวข้อ ใช้ Markdown Headings (####) และ Bullet points

    #### Insight:
    - วิเคราะห์โครงสร้างยอดขาย โดยระบุว่าช่วงราคาไหนคือ **"Mass Hero"** (ยอดขายสูงสุดและบิลเยอะ)
    - เปรียบเทียบปีปัจจุบันกับ SPLY ว่าลูกค้า Mass ไหลไปทาง Range สูงขึ้นหรือต่ำลง
    - มองหาโอกาสใน Range ถัดไปที่อยู่ติดกันกับ Mass Hero (Next Logical Step)
    - *ห้ามพูดถึง Range สูงที่เป็น Niche Market (บิลน้อย)*

    #### Action:
    - เสนอกลยุทธ์ดันยอดขาย โดยเน้นการ **"เขย่งยอด (Stretch)"** จาก Mass Hero ไปสู่ **Range ถัดไปที่อยู่ติดกันเท่านั้น**
    - ตัวอย่าง: หากคนส่วนใหญ่อยู่ 149-198 บาท ให้เสนอ Action เพื่อดันให้ถึง 199 หรือ 249 บาท (ห้ามเสนอให้ดันไป 1,000+)
    - เน้นกลยุทธ์เพิ่มจำนวนชิ้นต่อบิล (Basket Size) เพื่อให้ผ่านเกณฑ์ขั้นต่ำของ Range ถัดไป

    #### Suggest Basket Size:
    - แนะนำ **"Minimum Spend threshold"** สำหรับทำ GWP
    - **สูตรการคิด:** ให้เลือกตัวเลขที่เป็น **"ขอบล่าง (Lower Bound)"** ของ **Basket Range ถัดไป** ที่อยู่ติดกับ Mass Hero Range
    - *เหตุผล:* เพื่อจูงใจให้ลูกค้ากลุ่มใหญ่ที่สุด (Mass) ยอมจ่ายเพิ่มอีกนิดเพื่อข้ามไปอีก Step หนึ่ง (Achievable Goal)
    - **ห้าม** แนะนำตัวเลขที่สูงเกิน 1.5 เท่าของ ABR ของกลุ่ม Mass Hero
    """
