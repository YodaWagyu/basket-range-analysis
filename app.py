...
def generate_ai_insight(df, category, brand):
    ...
    prompt = f"""
    คุณคือ Business Data Analyst ที่แม่นยำเรื่อง logic และอธิบายด้วยภาษาพูดเข้าใจง่าย
    ช่วยวิเคราะห์ Basket Range สำหรับ Category = {category}, Brand = {brand}

    Data (ใช้ตารางนี้ในการคำนวณทุกอย่าง):
    {lean_df.to_string(index=False)}

    ---
    Logic Definitions (ต้องทำตามนี้เท่านั้น):

    1. Hero Range:
    - คือช่วง Basket Range ที่มี % SHARE SALES ของปีปัจจุบัน (CURRENT) สูงที่สุด
    - ให้มองปีปัจจุบันเป็นหลัก ถ้าต้องเทียบให้เทียบกับ SPLY (ปีก่อน)

    2. Traffic Generator:
    - คือช่วง Basket Range ที่ "ต่ำกว่า" Hero Range
    - เลือกช่วงที่มี % SHARE BILLS ของปีปัจจุบันค่อนข้างสูง และมีความหมายเชิงธุรกิจ
    - ถ้าใกล้เคียงกันให้เลือกช่วงที่อยู่ติดกับ Hero Range ก่อน

    3. The Goal:
    - เป้าหมายคือดันลูกค้าจาก Traffic -> Hero และจาก Hero -> Range ถัดไปที่สูงขึ้น
    - ห้ามแนะนำให้ดันลูกค้าลงไปซื้อของถูกกว่าเดิม

    ---
    Constraints (ข้อควรระวัง):

    1. Data Accuracy:
    - แยก SPLY (ปีก่อน) และ CURRENT (ปีนี้) ให้ชัด
    - เวลาพูดถึงการเปลี่ยนแปลงให้บอกว่ามากขึ้นหรือน้อยลงกี่ M THB หรือกี่ percentage point (ถ้าจำเป็น)

    2. Context:
    - ห้ามเอายอดขายรวมทั้งหมวด เช่น 50M THB ไปพูดเหมือนเป็นเป้าหมายของลูกค้าหนึ่งคน
    - ให้โฟกัสที่พฤติกรรมลูกค้าในแต่ละ Basket Range เป็นหลัก

    3. Human-readable Numbers:
    - ยอดขายให้ใช้หน่วย M THB เช่น 154M THB (ปัดทศนิยม 1 ตำแหน่งถ้าจำเป็น)
    - จำนวนบิลให้ใช้หน่วย M Bills หรือ K Bills เช่น 1.4M Bills, 740K Bills

    4. Style:
    - ใช้ภาษาพูดง่าย ๆ สั้น กระชับ เหมือนอธิบายให้คนการตลาดฟัง
    - ห้ามใช้ตัวหนา หรือเอียง ห้ามใส่สัญลักษณ์ * หรือ ** ในคำตอบ
    - ไม่ต้องใส่คำอธิบายยาวเกินไป เน้น key point ที่เอาไป action ได้จริง

    ---
    รูปแบบการตอบ (Output Format):

    ตอบเป็นภาษาไทย แบ่งเป็น 3 หัวข้อเท่านั้น ใช้ Markdown Headings และ bullet point ตามนี้เป๊ะ

    #### Insight
    - ระบุว่า Hero Range คือช่วงราคาไหน (อ้างอิงปีปัจจุบัน) พร้อมยอดขายประมาณกี่ M THB และสัดส่วน % SHARE SALES โดยรวม
    - ระบุว่า Traffic Generator คือช่วงไหน พร้อมจำนวนบิลประมาณกี่ K/M Bills และสัดส่วน % SHARE BILLS
    - เปรียบเทียบ Hero Range ปีนี้กับปีก่อนว่าขึ้นหรือลงประมาณกี่ M THB และตีความสั้น ๆ ว่าลูกค้ากองอยู่ตรงไหน หรือเริ่มขยับขึ้นหรือยัง

    #### Action
    - Action 1 (Fix Traffic): เสนอวิธีเปลี่ยนลูกค้ากลุ่ม Traffic Generator ให้ขยับยอดซื้อขึ้นมาปิดบิลใน Hero Range (เช่น โปรชิ้นเล็กเติมเงินอีกนิดให้ถึง)
    - Action 2 (Stretch Hero): เสนอวิธีดันลูกค้ากลุ่ม Hero Range ให้ขยับไป Range ถัดไปที่สูงขึ้น (เช่น จัดเซต, bundle, ของแถมเมื่อถึงยอดที่สูงกว่า)
    - ให้เขียนแบบเป็น bullet สั้น ๆ ชัดเจน เน้น idea ที่เอาไปทำโปรโมชันได้เลย

    #### Suggest Basket Size
    - แนะนำตัวเลขเดียวสำหรับใช้เป็นยอดขั้นต่ำทำ GWP (Gift With Purchase)
    - Logic: ให้เลือกเป็น "ราคาเริ่มต้น (lower bound) ของ Basket Range ถัดไป" ที่อยู่ติดกับ Hero Range
    - อธิบายเหตุผลสั้น ๆ 1–2 บรรทัด ว่าทำไมตัวเลขนี้ถึงเหมาะ เช่น ท้าทายแต่ลูกค้าส่วนใหญ่เอื้อมถึงได้

    อย่าพูดอะไรนอกเหนือจากหัวข้อ Insight, Action, Suggest Basket Size
    """
