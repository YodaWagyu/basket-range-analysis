...
def generate_ai_insight(df, category, brand):
    ...
    prompt = f"""
    คุณคือ Business Data Analyst ที่เน้นกลยุทธ์ Pareto (80/20) และการสื่อสารที่ "อ่านง่ายที่สุด"
    ...
    ---
    ### ⚠️ กฎการแสดงผล (Strict Formatting Rules):
    1. **Clean Text:** ห้ามใช้เครื่องหมาย `**`, `__`, หรือ Markdown formatting ในเนื้อหาเด็ดขาด (ยกเว้นหัวข้อหลัก ####) ต้องการให้อ่านสบายตาที่สุด
    2. **Human Numbers:**
       - ยอดขายหลักล้าน: ให้ตอบเป็นหน่วย "ล้านบาท" (เช่น 396.23 ล้านบาท -> "396 ล้านบาท")
       - ยอดขาย/บิลหลักพัน/แสน: ให้ใส่ comma คั่น (เช่น 1,500) และ **ตัดทศนิยมทิ้งทั้งหมด**
    3. **No Robot Speak:** ใช้ภาษาธุรกิจที่เข้าใจง่าย กระชับ ตรงประเด็น ไม่ต้องลอกตัวเลขทุกหลักมาใส่

    ---
    ### 🧠 วิธีคิดวิเคราะห์ (Logic Constraints):
    1. **Focus on Mass:** ให้ความสำคัญสูงสุดกับ Basket Range ที่มี **% Share Sales สะสมรวมกันสูงสุด 80% แรก** (Mass Market) เท่านั้น ตัด Range ที่คนซื้อน้อยๆ (% Share Bills < 5%) ทิ้งไป
    2. **GWP Logic:** การแนะนำยอดซื้อขั้นต่ำสำหรับของแถม (GWP) ให้ใช้ **"ราคาเริ่มต้นของ Range ถัดไป"** ที่อยู่ติดกับกลุ่ม Mass Hero เพื่อดึงให้ลูกค้าเขย่งยอดซื้อ (Stretch) ห้ามมโนตัวเลขที่ไม่มีในตาราง

    ---
    ### รูปแบบการตอบ (Output Structure):
    ตอบเป็นภาษาไทย แบ่งเป็น 3 หัวข้อ (ใช้ #### นำหน้า) และใช้ Bullet points

    #### Insight
    - ระบุว่าช่วงราคาไหนคือ "Mass Hero" (กลุ่มที่ยอดขายและบิลเยอะที่สุด) และทำเงินไปกี่ "ล้านบาท"
    - เปรียบเทียบเทรนด์ปีล่าสุดกับปีก่อนหน้าว่าลูกค้ากลุ่ม Mass ขยับไปซื้อของแพงขึ้นหรือถูกลง (แบบสรุปสั้นๆ)
    - ชี้เป้า "โอกาสถัดไป" คือ Range ราคาไหนที่ควรดันลูกค้าไปให้ถึง

    #### Action
    - เสนอกลยุทธ์ 2-3 ข้อ เพื่อดันลูกค้าจากกลุ่ม Mass Hero ให้ขยับไป Range ถัดไป (Next Step)
    - เน้นวิธีเพิ่มจำนวนชิ้น หรือจัดเซต เพื่อให้ยอดถึงเกณฑ์ของ Range ถัดไป

    #### Suggest Basket Size
    - แนะนำตัวเลข "ยอดซื้อขั้นต่ำ" (Minimum Spend) สำหรับทำโปรโมชั่น GWP
    - **ให้ใช้ตัวเลขที่เป็น "ขอบล่าง" ของ Basket Range ถัดไป** (จากกลุ่ม Mass Hero) 
    - อธิบายเหตุผลว่าทำไมยอดนี้ถึงจูงใจลูกค้ากลุ่ม Mass ได้ (Achievable Goal)
    """
