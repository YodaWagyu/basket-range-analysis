...
def generate_ai_insight(df, category, brand):
    ...
    1. **Data Accuracy:** แยกแยะปีเก่า (SPLY) และปีปัจจุบัน (CURRENT) ให้ถูกต้องแม่นยำ ห้ามสลับตัวเลข
    2. **Focus on Mass:** ให้ความสำคัญสูงสุดกับ Basket Range ที่มี **% Share Sales รวมกันสะสมอยู่ที่ 70-80% แรกเท่านั้น** (กลุ่ม Mass Market)
    3. **Ignore Niche:** ห้ามนำ Basket Range ที่มี **% Share Bills ต่ำกว่า 5%** มาวิเคราะห์เป็น Insight หลัก (ถือเป็น Niche ที่ไม่มีผลต่อภาพรวม)
    4. **Human-Readable Numbers (English Units):**
       - **ห้าม** พิมพ์ตัวเลขละเอียดทุกหลัก (เช่น 1,756,898)
       - **ต้อง** ปัดเศษตัวเลขและใช้หน่วยสากล (M = Million, K = Thousand)
       - **ยอดขาย (Sales):** ใช้หน่วย **M THB** (เช่น 306M THB)
       - **จำนวนบิล (Bills):** ใช้หน่วย **M Bills** หรือ **K Bills** (เช่น 1.7M Bills, 960K Bills)
    ...
    #### Insight
    - ระบุว่าช่วงราคาไหนคือ "Hero Range" (ยอดขายสูงสุด) และช่วงราคาไหนคือ "Traffic Generator" (บิลเยอะแต่ยอดขายรองลงมา)
    - เปรียบเทียบ Hero Range ของปีปัจจุบัน (CURRENT) กับปีก่อน (SPLY) ว่าเติบโตอย่างไร (ใช้หน่วย M/K)
    - วิเคราะห์พฤติกรรมลูกค้าว่ามีการ Trade up หรือ Trade down ในกลุ่ม Mass อย่างไร

    #### Action
    - เสนอวิธีขยับลูกค้าจากกลุ่ม Traffic Generator ให้ขึ้นมาสู่กลุ่ม Hero Range
    - ห้ามเสนอ action สำหรับกลุ่ม Range สูงที่ยอดขายน้อย (Niche)
    - ระบุตัวเลขเป้าหมายที่ต้องการให้ลูกค้าซื้อเพิ่ม (อิงจากขอบเขตของ Range ถัดไป)

    #### Suggest Basket Size
    - แนะนำ **Range เดียว** หรือ **ตัวเลขเดียว** สำหรับทำ GWP (Gift With Purchase)
    - **Logic:** เลือกตัวเลขที่เป็น "ราคาเริ่มต้น (Lower Bound) ของ Range ถัดไป" ที่อยู่ติดกับ Hero Range
    - *เหตุผล:* เพื่อดึงลูกค้า Mass ให้จ่ายเพิ่มอีกนิดเพื่อข้าม Step (Achievable Goal)
    """
