...
def generate_ai_insight(df, category, brand):
    ...
    1. **โฟกัสเฉพาะกลุ่ม Mass (The Core):** ให้ความสำคัญสูงสุดกับ Basket Range ที่มี **% Share Sales รวมกันสะสมอยู่ที่ 70-80% แรกเท่านั้น** (มักจะอยู่ในช่วงกลางๆ)
    2. **กำจัด Noise:** ห้ามให้ค่า หรือห้ามหยิบ Basket Range ที่มี **% Share Bills ต่ำกว่า 5%** มาวิเคราะห์เป็น Insight หลัก แม้ว่า ABR จะสูงแค่ไหนก็ตาม (เพราะถือเป็น Outlier/Niche ที่ไม่มีผลต่อยอดรวม)
    3. **ความคุ้มค่า (Efficiency):** ให้มองหา Range ที่ % Share Sales > % Share Bills (ทำน้อยได้มาก) ว่าอยู่ที่ช่วงราคาไหน
    4. **วิเคราะห์ปีล่าสุดเทียบ SPLY:** ดู Trend ว่าลูกค้าไหลไปที่ Range ที่สูงขึ้นหรือต่ำลง

    ---
    ### รูปแบบการตอบ (Output Format):
    ตอบเป็นภาษาไทย แบ่งเป็น 3 หัวข้อ ใช้ Markdown Headings (####) และ Bullet points ห้ามใช้ตัวหนา (**) ในเนื้อหา

    #### Insight:
    - ขอ 3-4 ข้อ ที่เน้น "โครงสร้างยอดขายหลัก"
    - ระบุว่า Basket Size ช่วงไหนคือ "Hero Range" (ยอดขายสูงสุด) และช่วงไหนคือ "Traffic Generator" (บิลเยอะแต่ยอดน้อย)
    - วิเคราะห์พฤติกรรมการขยับของลูกค้า (Trade up/down) ในกลุ่ม Mass เท่านั้น
    - *ห้ามพูดถึง Range ราคาสูงที่มีจำนวนบิลน้อย*

    #### Action:
    - ขอ 2-3 ข้อ ที่ Actionable และเน้น Impact สูงสุด
    - **กลยุทธ์ 20/80:** เสนอวิธีขยับลูกค้าจากกลุ่ม Traffic (Range ต่ำ) ให้ขึ้นมาสู่กลุ่ม Hero (Range กลาง) 
    - ห้ามแนะนำให้โฟกัสกลุ่ม Range สูงที่ยอดขายน้อย เพราะ Effort สูงแต่ Impact ต่ำ
    - ห้ามแนะนำกว้างๆ เช่น "เพิ่มโปรโมชั่น" ต้องระบุว่า "กระตุ้นให้ซื้อเพิ่มเพื่อแตะยอด X บาท"

    #### Suggest Basket Size:
    - แนะนำ **Range เดียว** หรือ **ช่วงราคาที่เชื่อมต่อกัน** ที่เหมาะสมที่สุดสำหรับการทำ GWP (Gift With Purchase)
    - หลักการเลือก: ต้องเป็นยอดที่สูงกว่า ABR เฉลี่ยของกลุ่ม Mass เล็กน้อย เพื่อดึงให้ลูกค้าเขย่งยอดซื้อ (Stretch goal) แต่ต้องไม่สูงจนเอื้อมไม่ถึง
    """
