...
def generate_ai_insight(df, category, brand):
    ...
    prompt = f"""
    คุณคือ Business Data Analyst ที่เชี่ยวชาญด้าน Pricing Strategy และ 80/20 Rule
    หน้าที่ของคุณคือวิเคราะห์ Basket Range สำหรับ Category = {category}, Brand = {brand} ให้แม่นยำและอ่านง่ายที่สุด

    Data Source (ยึดตามตารางนี้เท่านั้น):
    {lean_df.to_string(index=False)}

    ---
    ### 🧠 Logic การวิเคราะห์ (ต้องทำตามลำดับนี้):
    1.  **หา "Hero Range" ก่อน:**
        - คือช่วงราคาที่มี **% SHARE SALES สูงที่สุด** ในปีปัจจุบัน (CURRENT)
        - นี่คือกลุ่มลูกค้าหลักที่ทำเงินให้แบรนด์
    2.  **หา "Traffic Generator":**
        - คือช่วงราคาที่ **"ต่ำกว่า"** Hero Range ที่มีจำนวนบิล (% SHARE BILLS) สูงรองลงมาจาก Hero
        - (ถ้า Hero คือช่วงต่ำสุด ให้ถือว่า Traffic Generator คือช่วงเดียวกัน)
    3.  **หา "Next Step":**
        - คือช่วงราคาที่ **"สูงกว่า"** Hero Range ขึ้นไป 1 ขั้น (ติดกัน)
        - นี่คือเป้าหมายที่เราจะดันลูกค้าขึ้นไป

    ### ⚠️ ข้อควรระวัง (Strict Constraints):
    1.  **Column Check:** เช็คให้ดีว่ากำลังอ่านค่าจากช่อง **CURRENT (ปีปัจจุบัน)** หรือ **SPLY (ปีก่อน)** ห้ามสลับตัวเลข
    2.  **Format:**
        - **ห้ามใช้ตัวหนา**, ห้ามใช้ตัวเอียง, ห้ามมี markdown symbols (เช่น *, **) ในเนื้อหาเด็ดขาด
        - ยอดขาย: ใช้หน่วย **M THB** (เช่น 396M THB)
        - จำนวนบิล: ใช้หน่วย **M Bills** หรือ **K Bills** (เช่น 1.7M Bills, 960K Bills)

    ---
    ### รูปแบบการตอบ (Output Template):
    ตอบเป็นภาษาไทย แบ่งเป็น 3 หัวข้อ (ใช้ ####) และ Bullet points

    #### Insight
    - Hero Range คือช่วงราคาไหน (ระบุยอดขาย M THB และ % Share Sales ของปีปัจจุบัน)
    - Traffic Generator คือช่วงราคาไหน (ระบุจำนวนบิล K/M Bills ของปีปัจจุบัน)
    - เทรนด์ Hero Range: เทียบปีปัจจุบัน (CURRENT) กับปีก่อน (SPLY) ว่ายอดขายเติบโตขึ้นหรือลดลงกี่ M THB
    - สรุปพฤติกรรม: ลูกค้าส่วนใหญ่ยังกองอยู่ที่ Hero หรือเริ่ม Trade up ไปช่วงที่แพงขึ้นแล้ว?

    #### Action
    - Action 1 (Push Traffic up): เสนอวิธีจูงใจคนซื้อกลุ่ม Traffic Generator (ของถูก) ให้ขยับยอดซื้อเพิ่มเพื่อเข้าสู่ Hero Range
    - Action 2 (Stretch Hero up): เสนอวิธีจูงใจคนซื้อกลุ่ม Hero Range ให้ซื้อเพิ่มอีกนิดเพื่อแตะ **Next Step** (ช่วงราคาถัดไป)
    - *หมายเหตุ:* เน้นกลยุทธ์ Basket Size เช่น การจัดเซต (Bundling) หรือซื้อเพิ่มเพื่อรับสิทธิ์

    #### Suggest Basket Size
    - แนะนำตัวเลข **"ยอดซื้อขั้นต่ำ" (Minimum Spend)** สำหรับทำ GWP (Gift With Purchase)
    - **สูตรการเลือก:** ให้ใช้ตัวเลขที่เป็น **"ราคาเริ่มต้น (Lower Bound)" ของ Next Step Range** (ช่วงราคาที่สูงกว่า Hero Range 1 ขั้น)
    - เหตุผล: อธิบายสั้นๆ ว่ายอดนี้ท้าทายแต่ลูกค้า Hero เอื้อมถึง (Achievable Goal)
    """
