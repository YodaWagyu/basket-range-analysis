...
def generate_ai_insight(df, category, brand):
    ...
    prompt = f"""
    คุณคือ Business Data Analyst. หน้าที่คือวิเคราะห์ข้อมูลจากตารางด้านล่าง แล้วเติมข้อมูลลงในแบบฟอร์มให้ถูกต้อง

    Data Source:
    {lean_df.to_string(index=False)}

    --------------------------------
    ⚠️ ขั้นตอนการทำงาน (Logic):
    1. **ระบุ Hero Range:** ค้นหาแถวที่มี '% SHARE SALES' สูงที่สุด ในฝั่ง **CURRENT (ปีปัจจุบัน)** -> จำค่า Basket Range, Sales, และ % Share ไว้
    2. **ระบุ Traffic Generator:** ค้นหาแถวที่มี '% SHARE BILLS' สูงที่สุด ในฝั่ง **CURRENT** (ที่ราคาต่ำกว่า Hero) -> จำค่า Basket Range, Bills, และ % Share ไว้
    3. **คำนวณการเปลี่ยนแปลง:** เอา Sales ของ Hero Range (CURRENT) ลบด้วย Sales ของ Hero Range (SPLY)

    ⚠️ กติกาฟอร์แมต (Formatting):
    - **ห้าม** พิมพ์คำสั่งในวงเล็บลงไปในคำตอบ ให้แทนที่ด้วยตัวเลขจริงเท่านั้น
    - **ห้าม** ใช้ตัวหนา (Bold)
    - ยอดขาย = หน่วย M THB (เช่น 43.10 M THB)
    - จำนวนบิล = หน่วย K Bills หรือ M Bills (เช่น 117.90 K Bills)

    --------------------------------
    แบบฟอร์มคำตอบ (ให้เติมข้อมูลจริงลงในวงเล็บ [ ] เท่านั้น):

    #### Insight
    - Hero Range: [ใส่ชื่อช่วงราคาตรงนี้] (Sales: [ใส่ตัวเลขยอดขาย] M THB, Share: [ใส่ตัวเลข %]%)
    - Traffic Generator: [ใส่ชื่อช่วงราคาตรงนี้] (Bills: [ใส่ตัวเลขบิล] K Bills, Share: [ใส่ตัวเลข %]%)
    - การเปลี่ยนแปลง: ยอดขาย Hero Range ปีนี้เทียบกับปีก่อน เปลี่ยนแปลง [ใส่ยอดขายที่เพิ่มหรือลด] M THB

    #### Action
    - Action 1: ดึงลูกค้าจาก [ใส่ชื่อช่วง Traffic Generator] ให้ซื้อเพิ่มเพื่อขยับขึ้นมาที่ [ใส่ชื่อช่วง Hero Range] โดยจัดโปรโมชั่นเพิ่มยอดต่อบิล
    - Action 2: ดึงลูกค้าจาก [ใส่ชื่อช่วง Hero Range] ให้ซื้อเพิ่มเพื่อขยับไปที่ [ใส่ชื่อช่วงราคาถัดไปที่สูงขึ้น] โดยใช้กลยุทธ์จัดเซตสินค้า

    #### Suggest Basket Size
    - แนะนำ GWP ที่ยอด: [ใส่ตัวเลขราคาเริ่มต้นของ Range ถัดไป] THB
    - เหตุผล: เป็นราคาเริ่มต้นของ Range ถัดไป ซึ่งท้าทายแต่ลูกค้ากลุ่ม Hero Range ยังเอื้อมถึง
    """
