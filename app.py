...
def generate_ai_insight(df, category, brand):
    ...
    prompt = f"""
    คุณคือ Business Data Analyst ที่ละเอียดรอบคอบสูงสุด
    หน้าที่: สรุปข้อมูล Basket Range ของ Category = {category}, Brand = {brand}

    Data Source (ตารางนี้แบ่งเป็น 2 ฝั่งชัดเจน):
    {lean_df.to_string(index=False)}

    --------------------------------
    คำสั่งบังคับ (Critical Rules - Do Not Violate):

    1. **ห้ามใช้ข้อมูลผิดปีเด็ดขาด (COLUMN MAPPING):**
    - ตารางฝั่งซ้าย (SPLY) = อดีต -> เอาไว้ดูเทียบเท่านั้น ห้ามเอามาตอบเป็นสถานะปัจจุบัน
    - ตารางฝั่งขวา (CURRENT) = ปัจจุบัน -> **ข้อมูล Insight ทั้งหมดต้องมาจากฝั่งนี้เท่านั้น**
    - **Hero Range** ต้องดูจาก % SHARE SALES ของฝั่ง **CURRENT**
    - **Traffic Generator** ต้องดูจาก % SHARE BILLS ของฝั่ง **CURRENT**

    2. **ห้ามคำนวณตัวเลขเอง (NO CALCULATION):**
    - ให้ "คัดลอก" ตัวเลข % SHARE SALES และ % SHARE BILLS จากในตารางมาตอบตรง ๆ ห้ามกดเครื่องคิดเลขใหม่
    - ถ้าในตารางเขียน 61.44% ก็ต้องตอบ 61.44% ห้ามตอบเลขอื่น

    3. **ห้ามใช้ตัวหนา (NO BOLD):**
    - ห้ามพิมพ์เครื่องหมายดอกจัน (**) ในคำตอบเด็ดขาด
    - ขอเป็น Plain Text ล้วน ๆ อ่านสบายตา

    4. **หน่วยตัวเลข (UNITS):**
    - ยอดขาย: ใช้หน่วย M THB (ทศนิยม 1-2 ตำแหน่ง)
    - จำนวนบิล: ใช้หน่วย K Bills หรือ M Bills (ทศนิยม 1-2 ตำแหน่ง)

    --------------------------------
    รูปแบบการตอบ (ตอบตาม Template นี้เท่านั้น):

    #### Insight
    - Hero Range: ระบุช่วงราคา (Sales: ... M THB, Share: ...% จากฝั่ง CURRENT เท่านั้น)
    - Traffic Generator: ระบุช่วงราคา (Bills: ... K/M Bills, Share: ...% จากฝั่ง CURRENT เท่านั้น)
    - การเปลี่ยนแปลง: ยอดขาย Hero Range ปีนี้ เทียบกับปีก่อน เพิ่มขึ้นหรือลดลงกี่ M THB

    #### Action
    - Action 1: เสนอวิธีดึงลูกค้า Traffic Generator (ช่วงราคาต่ำกว่า) ให้ขยับยอดซื้อขึ้นมาที่ Hero Range
    - Action 2: เสนอวิธีดึงลูกค้า Hero Range ให้ขยับยอดซื้อไปที่ Range ถัดไป (ช่วงราคาสูงกว่า 1 ขั้น)

    #### Suggest Basket Size
    - ตัวเลขแนะนำ GWP: ระบุราคาเดียว (ใช้ราคาเริ่มต้นของ Range ถัดไป ที่อยู่ติดกับ Hero Range)
    - เหตุผล: สั้น ๆ กระชับ
    """
