...
def generate_ai_insight(df, category, brand):
    ...
    # Separate SPLY and CURRENT year data
    years = sorted(lean_df['Year'].unique())
    if len(years) < 2:
        return "⚠️ Need at least 2 years of data for comparison"
    
    sply_year = years[0]
    curr_year = years[1]
    
    df_sply = lean_df[lean_df['Year'] == sply_year].copy()
    df_curr = lean_df[lean_df['Year'] == curr_year].copy()
    
    # Find Hero Range and Traffic Range from CURRENT year
    hero_range = df_curr.loc[df_curr['TotalSales'].idxmax(), 'Basket_Range']
    traffic_range = df_curr.loc[df_curr['TotalBills'].idxmax(), 'Basket_Range']
    
    # Get values for Hero Range
    hero_curr = df_curr[df_curr['Basket_Range'] == hero_range]
    hero_sply = df_sply[df_sply['Basket_Range'] == hero_range]
    ...
    comparison_df = df_sply[['Basket_Range', 'TotalSales', 'TotalBills']].rename(
        columns={'TotalSales': 'Sales_SPLY', 'TotalBills': 'Bills_SPLY'}
    ).merge(
        df_curr[['Basket_Range', 'TotalSales', 'TotalBills']].rename(
            columns={'TotalSales': 'Sales_CURR', 'TotalBills': 'Bills_CURR'}
        ),
        on='Basket_Range',
        how='outer'
    ).fillna(0)
    
    # Check if Traffic and Hero are the same range
    if traffic_range == hero_range:
        action_instruction = f"""
    #### Action
    💡 กลุ่มเป้าหมาย: ช่วง {hero_range} เป็นทั้ง Hero Range (ยอดขายสูงสุด) และ Traffic Generator (จำนวนบิลสูงสุด) ในเวลาเดียวกัน จึงเป็นกลุ่มที่มีศักยภาพสูงสุดในการเพิ่มยอด
    
    - 1. กระตุ้นให้ซื้อเพิ่ม: [แนะนำวิธีจัดโปรฯ หรือเซต/ของแถม เพื่อดันให้ลูกค้ากลุ่มนี้ซื้อเพิ่มไปแตะช่วง {next_range}]
    - 2. รักษาฐานลูกค้า: [แนะนำวิธีรักษาลูกค้ากลุ่มนี้ไม่ให้ลดลง เพราะเป็นกลุ่มหลักที่สร้างทั้งยอดและบิล]
        """
    else:
        action_instruction = f"""
    #### Action
    💡 กลุ่มเป้าหมาย: โฟกัส 2 กลุ่มหลัก - (1) ช่วง {traffic_range} มีคนซื้อเยอะ แต่ยอดยังไม่สูง (2) ช่วง {hero_range} ยอดขายสูงสุด ต้องดันให้ซื้อเพิ่ม
    
    - 1. ดันลูกค้ากลุ่ม {traffic_range}: [วิธีจัดโปรฯ เพื่อให้ขยับไปช่วง {hero_range} - กลุ่มนี้มีคนเยอะ ถ้าดันได้จะเพิ่มยอดเยอะ]
    - 2. ดันลูกค้ากลุ่ม {hero_range}: [วิธีจัดเซต/ของแถม ให้ซื้อเพิ่มไปช่วง {next_range} - กลุ่มนี้ยอดสูงอยู่แล้ว ต้องดันให้สูงขึ้นอีก]
        """
    
    prompt = f"""
    คุณคือเพื่อนคู่คิดด้านธุรกิจค้าปลีก ที่เก่งเรื่องการสรุปตัวเลขให้เข้าใจง่ายและอ่านสบายตา
    
    📦 สินค้า: {category} - {brand}
    
    📊 ข้อมูลเปรียบเทียบ SPLY vs CURRENT:
    ...
    1. **ห้ามคำนวณเอง** ใช้เฉพาะตัวเลขที่ให้มาข้างบน
    2. **ห้ามพูดกำกวมทั้งสองด้าน** เช่น "ลูกค้ากล้าจ่ายแพงขึ้น แต่ก็ยังให้ความสำคัญกับราคาที่ถูก" ← ห้าม!
    3. **ต้องเลือกทิศทางชัดเจน** อย่างใดอย่างหนึ่ง: 
       - ถ้ายอดขาย Hero Range เพิ่ม = ลูกค้ากล้าจ่ายแพงขึ้น
       - ถ้ายอดขาย Hero Range ลด = ลูกค้าเน้นประหยัด/ราคาถูกลง
    4. **ห้ามใช้ตัวหนา (**...**)** ในเนื้อหา
    5. **ห้ามเขียนเป็นย่อหน้า** ใช้ Bullet points เท่านั้น
    ...
    #### Insight
    - 🏆 Hero Range (ขายดีสุด): {hero_range} ยอดขายรวม [แปลง Sales_CURR เป็น M THB] M THB
    - 🚦 Traffic Generator (ดึงคนเก่งสุด): {traffic_range} จำนวนบิล [แปลง Bills_CURR เป็น K Bills] K Bills
    - 📈 เทรนด์ปีนี้: ช่วง {hero_range} ยอดขาย [เพิ่มขึ้น/ลดลง] [ระบุจำนวน M THB] เมื่อเทียบกับปีก่อน
    - 💡 สรุปพฤติกรรม: [เลือกทิศทางเดียว ห้ามพูดสองด้าน - ดูจาก Trend ของ Hero Range]
    
{action_instruction}
    
    #### Suggest Basket Size
    - 🎯 ยอดซื้อเป้าหมาย: {next_price}+ บาท
    - � [อธิบายสั้นๆ แบบสบายๆ ว่าทำไมถึงแนะนำยอดนี้ โดยอาจพิจารณาจาก: (1) ค่าเฉลี่ยในช่วง {hero_range} อยู่ที่ {hero_abr_curr:.0f} บาท ขาดอีก {gap:.0f} บาท ก็เข้าช่วง {next_range}, (2) ความคุ้มค่าของของแถม, (3) จิตวิทยาลูกค้า - ให้เลือกมุมที่เหมาะสมที่สุดและเขียนแบบเป็นธรรมชาติ ไม่ต้องขึ้นต้นด้วย "เหตุผล:" หรือ "เพราะว่า"]
    """
    ...
    try:
        res = client.chat.completions.create(
            model="gpt-4o-mini",
            messages=[
                {"role": "system", "content": "คุณคือที่ปรึกษาด้านการค้าปลีกที่อธิบายข้อมูลได้ง่ายๆ เหมือนเพื่อนคุย ไม่ใช้คำเทคนิค และต้องเลือกทิศทางชัดเจน ห้ามพูดกำกวมทั้งสองด้าน"},
